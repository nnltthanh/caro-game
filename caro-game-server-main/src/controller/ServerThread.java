        ...
        try {
            ...
            while (!isClosed) {
                ...
                if(messageSplit[0].equals("client-verify")){
                    Player user1 = userDAO.verifyPlayer(new Player(messageSplit[1], messageSplit[2]));
                    if(user1==null)
                ...
                if(messageSplit[0].equals("register")){
                   ...
                   if(checkdup) write("duplicate-username,");
                       ...
                       Player userRegister = new Player(messageSplit[1], messageSplit[2], messageSplit[3], messageSplit[4]);
                       userDAO.addUser(userRegister);
                       Player userRegistered = userDAO.verifyPlayer(userRegister);
                       this.user = userRegistered;
                       userDAO.updateToOnline(this.user.getID());
                       Server.serverThreadBus.boardCast(clientNumber, "chat-server,"+this.user.getNickname()+" đang online");
                       Server.admin.addMessage("["+this.user.getID()+"] "+this.user.getNickname()+ " đang online");
                ...
                if(messageSplit[0].equals("offline")){
                    ...
                    this.user=null;
//                    Server.serverThreadBus.remove(this.user.getID());
                }
                //Xử lý xem danh sách bạn bè
                if(messageSplit[0].equals("view-friend-list")){
                    List<Player> friends = userDAO.getListFriend(this.user.getID());
...
//                    user.setNumberOfwin(user.getNumberOfwin()+1);
                ...
                if(messageSplit[0].equals("lose")){
                    ...
                    //add
//                    room.getCompetitor(clientNumber).user.setNumberOfwin(room.getCompetitor(clientNumber).user.getNumberOfwin()+1);
                    room.increaseNumberOfGame();
...
//                	room.getCompetitor(clientNumber).user.setNumberOfDraw(room.getCompetitor(clientNumber).user.getNumberOfDraw()+1);
//                	
